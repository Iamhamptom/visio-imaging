  ...
  try {
    ...
    if (!file) {
      return NextResponse.json(
        { error: "No file provided. Send a prescription image as 'file' in multipart form data." },
        { status: 400 }
      );
    }

    const uploaded = await uploadFile(file, "prescription");
    ...
    try {
      ...
      const result = await analyzePrescription(base64, file.type);
